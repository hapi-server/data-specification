...
| `location`          | object             | **Optional** A way to specify where a dataset's measurement were made. See [`location` Details](#3617-location-and-geoLocation-details) for an explanation of the two ways to indicate location: one for a fixed location and one for measurements whose location changes with time.
| `geoLocation`          | array of double | **Optional** An alternate, shorthand way of indication fixed location for a dataset in Earth-based coordaintes. The values in the array and their units and coordinate frame are meant to match what is used in GeoJSON (geojson.org). See [`location` Details](#3617-location-and-geoLocation-details).
