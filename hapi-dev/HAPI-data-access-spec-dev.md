...
| units               | string               | **Required** The units for the data values represented by this parameter. For dimensionless quantities, the value can be ‘dimensionless’ or `null`. For `isotime` parameters, the type must be `UTC`. If a paramter is an array quantity, these units will be applied to all elements in the array. If the units are different for elements in an array, use the `unitsArray` keyword below. So this keyword is required for scalars, and for arrays, one of `units` or `unitsArray` is required. |
| unitsArray               | string array               | **Optional** For an array parameter with mixed units (different units for different elements in the array), this array allows the units of each element in the array to be separately specified. Do not use if using the `units` keyword. If the `units` keyword is not given for an array parameter, this is required. |
...
| label               | string               | **Optional** A word or very short phrase that could serve as a label for this parameter (as on a plot axis or in a selection list of parameters). Intended to be less cryptic than the parameter name.  This label will be applied to all elements of an array parameter. Do not use this if `labelArray` is used. |
| labelArray          | string array         | **Optional** Labels for individual elements in an array parameter. Each should be a word or very short phrase. Only use with array parameters and only if not using the `label` keyword.  |
