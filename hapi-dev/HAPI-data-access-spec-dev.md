...
Version 2.0.1-dev \| Heliophysics Data and Model Consortium (HDMC) \|

**This is the development version of the HAPI Data Access Specification.**

The most recent stable release is [Version 2.0.0](https://github.com/hapi-server/data-specification/tree/master/hapi-2.0.0).
...
| units               | string OR array of string | **Required** The units for the data values represented by this parameter. For dimensionless quantities, the value can be ‘dimensionless’ or `null`. For `isotime` parameters, the units must be `UTC`. If a paramter is a scalar, the units must also be a scalar. For an array parameter, a scalar units value means that the same units apply to all elements in the array. If the elements in the array parameter have different units, the `units` attribute can be an array to provide specific units strings for each element in the array. The shape of such a units array must match the `size` of the parameter. Ordering of multi-dimensional arrays of units trings is as discussed in the `size` attribute definition above. |
...
| label               | string OR array of string | **Optional** A word or very short phrase that could serve as a label for this parameter (as on a plot axis or in a selection list of parameters). Intended to be less cryptic than the parameter name.  If the parameter is a scalar, this label must be a scalar. If the parameter is an array, a scalar label or an array of string labels are allowed.  A scalar label will be applied to all elements in the array, whereas an array of label strings specifies a different label string for each element in the array parameter. The shape of the array of label strings must match the `size` attribute. Ordering of multi-dimensional arrays of label strings is as discussed in the `size` attribute definition above. |
